...
boxs.forEach(box => {
    box.addEventListener("click", () => {
        let dataTab = box.getAttribute("data-tab");
        ...
        if (!nbChoisi && !box.classList.contains("desactivate_box")) {
            ...
            let ligne = dataTab.substr(0, 1);
            let colonne = dataTab.substr(2, 1);
            ...
            if (!box.classList.contains("desactivate_box") && !box.classList.contains("active_echange") && !box.classList.contains("active")) {
                ...
                gains.forEach(gain => {
                    if (tabBox[ligne][colonne] == gain.getAttribute("data-id")) {
                        ...
                        setTimeout(() => {
                            let randomBanquier = Math.floor(Math.random() * 3) + 1;
                            switch (nbTour) {
                                case 5:
...
function genereGainAleatoire(array) {
    let tab = array;
    let randomGain = Math.floor(Math.random() * 24) + 1;
...
function infoMessage(info) {
    ...
    switch (info) {
        case "reste":
            message.innerHTML = "Choisissez un numéro à retirer, il vous en reste <span>" + reste + "</span> avant l'offre du banquier";
            break;
        case "dernier":
...
function calculBanquier() {
    ...
    let result = sommeTab / tabGainRestant.length;
...
function messageBanquier(nbPourDevise, nbReste, randomBanquier) {
    if (randomBanquier < 3) {
        let offre = calculBanquier() / nbPourDevise;
        offre = Math.round(offre);
        offre = new Intl.NumberFormat().format(offre)
